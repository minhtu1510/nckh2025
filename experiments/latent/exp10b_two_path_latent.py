"""
Experiment 10b LATENT: Two-Path Routing — DeDe + DualEncoder bị nhiễm
======================================================================

Tương tự Exp9 (Two-Path Routing) nhưng TOÀN BỘ hệ thống bị retrain trên poisoned data:
  - DeDe        retrain trên RAW poisoned  → threshold bị lệch
  - DualEncoder retrain trên poisoned data → latent space bị nhiễm
  - Stacking    train trên poisoned latent → classifier bị nhiễm

Câu hỏi: Khi defender retrain toàn bộ trên poisoned data,
         hệ thống Two-Path còn phòng thủ được không?

So sánh trực tiếp:
  Exp9  : DeDe(clean) + DualEnc(clean)  + Stack(clean/poison labels)
  Exp10b: DeDe(POISON)+ DualEnc(POISON) + Stack(poison latent)

Output format GIỐNG EXP9:
...
train_scenarios:
  clean       ← Exp9 baseline (clean DeDe + clean enc) — reuse Exp9 results
  poison_05   ← FULL SYSTEM POISONED at 5%
...
class TwoPathHybridLatent:
    ...
    def predict(self, X_raw, return_details=False):
        ...
        w_gan = np.sqrt(w_gan)  # <--- BƯỚC ĐIỀU CHỈNH THẦN TỐC
        
        non_trigger = ~trigger_mask
        if non_trigger.sum() > 0:
            X_lat = self.dual_enc.encode(X_raw[non_trigger])
            
            # Lấy xác suất từ cả 2 Chuyên gia
            p_std = self.std_stack.predict_proba(X_lat)[:, 1]
            p_gan = self.ganopt_stack.predict_proba(X_lat)[:, 1]
            
            # Khớp Trọng số động (Lỗi càng cao, Quyền lực của GAN-Opt càng lớn)
            w_g = w_gan[non_trigger]
            p_final = (1.0 - w_g) * p_std + w_g * p_gan
            
            pred[non_trigger] = (p_final >= 0.5).astype(int)

        pred[trigger_mask] = 1

        if return_details:
            ganopt_mask = (errs >= self.low_thr) & ~trigger_mask
